...
const ArticleCard: React.FC<ArticleCardProps> = ({ article }) => {
  return (
    <Card className="relative group flex justify-between items-start p-4 w-full gap-6">
      ...
      <Button className="absolute transition opacity-0 group-hover:opacity-100 bottom-5 md:left-1/3 md:px-10 left-[11.5rem] px-6">
        <Link to={`/article/${article.id}`}>Keep Reading</Link>
      </Button>
    </Card>
  );
};
